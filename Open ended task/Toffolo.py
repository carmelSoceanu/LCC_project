...
# example_files = ['NPI_cup(flag).wav', 'NPC_cake.wav', 'NPC_cup.wav', 'NPI_cake(clue).wav']
# example_files = files_df[files_df['stim_key'].str.contains('Ex', na=False)]['stim_file'].tolist()
...
def get_EEG(mat_data, participant, condition, avg_window=(0, len(time_axis))):
    ...
    # print(f'{participant}: {len(urevent)}, {len(stimuli_df)}, {len(event)}')
    ...
    # print(stimuli_data[stimuli_data['linguistic-group_div'] == 5]['stim_file'].tolist())
    # stimuli_data.to_pickle(filename)
