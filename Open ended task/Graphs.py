...
def plot_all_regressions_separated(datasets, results, figsize=(8, 12), save=True):
    ...
    def plot_regressions(points_list, title, suffix):
        ...
        num_cols = 2  # Define the number of columns you want in the grid
    ...
    plot_regressions(high_r_squared_points, 'Embedding Models Performance Correlations\n(R² > 0.15)','High')
    
    # Plot regressions for low R²
    plot_regressions(low_r_squared_points, 'Embedding Models Performance Correlations\n(R² ≤ 0.15)', 'Low')
...
def plot_dataset_comparison(datasets, results, filename=None):
    ...
    num_rows = 3
    num_cols = (num_datasets + num_rows - 1) // num_rows  # Calculate the number of columns needed
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(6 * num_cols, 9))  # Adjust figure size accordingly
    ...
    fig.suptitle('Model-Baseline Comparisons per Dataset\n(logscale, gray = baseline, label = # significant hypotheses)', fontsize=20, fontweight='bold')
...
def plot_testing_losses(results, dataset_name, participant, filename=None):
    ...
    plt.title(f'Average Testing Loss of Models\nOver {dataset_name} {participant}\n(dashed line = baseline)', fontsize=16, fontweight='bold')
